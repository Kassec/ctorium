 ...
 * Each contributed descriptor is translated into a runtime `Descriptor` (TypeId /
 * NameId interned, `Identity` stripped, `factoryMethodIdentity` resolved to a
 * `DescriptorId`) and appended here.
 *
 ...
 * by `at()` are therefore stable for the lifetime of the table.
 *
 * ### Indexing invariant
...
public:
     ...
     *
     * @param d Fully translated runtime descriptor.  Moved into storage.
     * @return Dense `DescriptorId` equal to the 0-based insertion index.
     */
    [[nodiscard]] DescriptorId append(Descriptor&& d) {
